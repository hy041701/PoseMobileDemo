...
object PoseFrameConverter {
    fun convert(
        ...
        val persons = mutableListOf<PosePersonData>()
        ...
        normalizedPoses.forEachIndexed {   //遍历每一个人，personIndex人的下标
                personIndex,
                normalizedLandmarks ->
            val worldLandmarks = worldPoses.getOrNull(personIndex)     //根据当前的personIndex，去worldPoses中找同一个人的world landmarks

            val landmarkList = normalizedLandmarks.mapIndexed {      //遍历这个人的33个关键点
                        landmarkIndex,
                        landmark ->
                    val worldLandmark = worldLandmarks?.getOrNull(landmarkIndex)
                    ...
                    PoseLandmarkData(
                        ...
                        y = landmark.y(),
                        z = landmark.z(),
                        worldX = worldLandmark?.x() ?: 0f,
                        worldY = worldLandmark?.y() ?: 0f,
                        worldZ = worldLandmark?.z() ?: 0f,
